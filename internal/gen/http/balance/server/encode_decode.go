...
// EncodeGetWithdrawalsResponse returns an encoder for responses returned by
...
// DecodeGetWithdrawalsRequest returns a decoder for requests sent to the
// balance GetWithdrawals endpoint.
func DecodeGetWithdrawalsRequest(mux goahttp.Muxer, decoder func(*http.Request) goahttp.Decoder) func(*http.Request) (*balance.GetWithdrawalsPayload, error) {
	return func(r *http.Request) (*balance.GetWithdrawalsPayload, error) {
		var (
			authorization string
			err           error
		)
		...
		payload := NewGetWithdrawalsPayload(authorization)
		if strings.Contains(payload.Authorization, " ") {
			// Remove authorization scheme prefix (e.g. "Bearer")
			cred := strings.SplitN(payload.Authorization, " ", 2)[1]
			payload.Authorization = cred
...
// EncodeGetWithdrawalsError returns an encoder for errors returned by the
// GetWithdrawals balance endpoint.
func EncodeGetWithdrawalsError(encoder func(context.Context, http.ResponseWriter) goahttp.Encoder, formatter func(ctx context.Context, err error) goahttp.Statuser) func(context.Context, http.ResponseWriter, error) error {
	encodeError := goahttp.ErrorEncoder(encoder, formatter)
	return func(ctx context.Context, w http.ResponseWriter, v error) error {
		var en goa.GoaErrorNamer
		if !errors.As(v, &en) {
